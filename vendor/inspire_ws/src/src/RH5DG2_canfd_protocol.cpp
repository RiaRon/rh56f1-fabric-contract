...
bool RH5DG2_canfd_Protocol::writeRegister(Device device, const std::string& reg_name, const std::vector<int>& values) {
...
std::pair<bool, std::vector<int>> RH5DG2_canfd_Protocol::readRegister(
    Device device,
    ...
    return {true, std::move(all_values)};
}

// 触觉数据读取：逻辑长度固定 68 字节，拆为多帧读取并拼接，然后按 485 版本算法解析
std::pair<bool, TouchDataResult> RH5DG2_canfd_Protocol::readTouchData(
    Device device,
    RingBuffer& ringBuffer,
    int version) {
