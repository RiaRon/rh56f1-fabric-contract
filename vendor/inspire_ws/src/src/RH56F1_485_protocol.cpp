...
// 写寄存器
bool RH56F1_485_Protocol::writeRegister(Device device, const std::string& reg_name, const std::vector<int>& values) {
...
// 读寄存器
std::pair<bool, std::vector<int>> RH56F1_485_Protocol::readRegister(Device device, RingBuffer& ringBuffer, const std::string& reg_name, size_t length) {
...
// 读取触觉数据
std::pair<bool, TouchDataResult> RH56F1_485_Protocol::readTouchData(Device device, RingBuffer& ringBuffer, int version) {
    std::ostringstream oss;
    auto logger = getLogger(); // 获取日志对象
    try {
        // 读取前清空环形缓冲，避免历史帧干扰当前解析
