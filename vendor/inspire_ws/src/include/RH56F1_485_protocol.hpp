...
public:
    ...
    // 重写父类虚函数
    bool writeRegister(Device device, const std::string& reg_name, const std::vector<int>& values) override;
    std::pair<bool, std::vector<int>> readRegister(Device device, RingBuffer& ringBuffer, const std::string& reg_name, size_t length) override;
    std::pair<bool, TouchDataResult> readTouchData(Device device, RingBuffer& ringBuffer, int version) override;
