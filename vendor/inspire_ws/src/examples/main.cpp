...
void deviceControlThread(
    ...
    while (g_running.load()) {
        try {
            ...
            bool writeSuccess = protocol->writeRegister(device, "angleSet", angles);
            
            // 2. 读取当前角度值（angleAct）
            auto [successRead, regValues] = protocol->readRegister(device, ringBuffer, "angleAct", 26);            
            
            // 3. 读取触觉数据（touchAct）
            auto [touchSuccess, touchData] = protocol->readTouchData(device, ringBuffer, 1);
