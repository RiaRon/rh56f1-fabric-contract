...
class RightArmEefController : public rclcpp::Node
{
public:
  RightArmEefController()
  : Node("right_arm_eef_controller")
  {
    declare_parameter<std::string>(
      "urdf_path", "/home/user/rl_ws/sim2real/urdf/openarm_tesollo_sensor/openarm_tesollo_sensor.urdf");
    declare_parameter<std::string>("root_link", "openarm_right_link0");
    declare_parameter<std::string>("tip_link", "palm_ee");
    declare_parameter<std::string>("joint_state_topic", "/joint_states");
    declare_parameter<std::string>("target_pose_topic", "/openarm/right_arm/eef_target");
    declare_parameter<std::string>(
    ...
    joint_state_sub_ = create_subscription<sensor_msgs::msg::JointState>(
      ...
      std::bind(&RightArmEefController::joint_state_callback, this, std::placeholders::_1));
    target_pose_sub_ = create_subscription<geometry_msgs::msg::PoseStamped>(
      target_pose_topic, 10,
      std::bind(&RightArmEefController::target_pose_callback, this, std::placeholders::_1));
    trajectory_pub_ = create_publisher<trajectory_msgs::msg::JointTrajectory>(trajectory_topic, 10);

    RCLCPP_INFO(
      get_logger(),
      "Right arm EEF controller ready. root=%s tip=%s target_topic=%s trajectory_topic=%s",
...
private:
  ...
  void target_pose_callback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
  {
    ...
    const auto seconds = rclcpp::Duration::from_seconds(trajectory_time_sec_);
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<RightArmEefController>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
