...
class FabricSpec:
    ...
    params: str
    world: dict
    scope: str = "side"                    # side | asset (one cspace covers both sides)
...
class AssetSpec:
    ...
    @property
    def canonical_joint_order(self) -> tuple[str, ...]:
        """Asset control order; for RH56F1 this is also the 26-D Fabric order."""
        return tuple(load_manifest(self)["control_joint_order"])

    @property
    def joint_groups(self) -> dict[str, tuple[str, ...]]:
        return {k: tuple(v) for k, v in canonical_joint_groups(self.canonical_joint_order).items()}
...
ASSETS = {
    ...
    "openarm_rh56f1_bi_rl": AssetSpec("openarm_rh56f1_bi_rl", "rh56f1", {
        "right": FabricSpec("OpenArmRh56f1PoseFabric", "openarm_rh56f1",
                            "openarm_rh56f1_pose_params.yaml",
                            {"filename": "open_tesollo_boxes_no_table"}, scope="asset"),
        "left": FabricSpec("OpenArmRh56f1LeftPoseFabric", "openarm_rh56f1",
                           "openarm_rh56f1_pose_params.yaml",
                           {"filename": "open_tesollo_left_boxes_no_table"}, scope="asset"),
    }),
}
...
CANONICAL_GROUPS = ("right_arm", "right_hand", "left_arm", "left_hand")


def canonical_joint_groups(joint_order) -> dict[str, list[str]]:
    """Partition canonical names without assuming a left-first or right-first layout."""
    groups = {name: [] for name in CANONICAL_GROUPS}
    for joint in joint_order:
        side = "right" if joint.startswith("r_") else "left" if joint.startswith("l_") else None
        kind = "arm" if "_aj_" in joint else "hand" if "_hj_" in joint else None
        if side is None or kind is None:
            raise ContractError(f"cannot assign canonical joint {joint!r} to a deployment group")
        groups[f"{side}_{kind}"].append(joint)
    return groups


def canonical_group_indices(joint_order, groups) -> dict[str, list[int]]:
    order = list(joint_order)
    return {name: [order.index(joint) for joint in joints] for name, joints in groups.items()}


def split_fabric_state(fabric: FabricCfg, q) -> dict[str, list[float]]:
    """Extract named groups from a canonical Fabric vector."""
    values = list(q)
    if len(values) != len(fabric.joint_order):
        raise ContractError(f"state has {len(values)} values, expected {len(fabric.joint_order)}")
    return {name: [values[i] for i in indices] for name, indices in fabric.group_indices.items()}


def assemble_fabric_state(fabric: FabricCfg, groups: dict, enabled_groups: dict | None = None) -> list[float]:
    """Assemble one canonical vector; disabled groups stay at parked_q (or home_q).

    Group values are addressed only through name-derived indices. A supplied value for an inactive
    group is ignored, so missing hardware cannot leak commands into its canonical slots.
    """
    enabled = ({name: True for name in fabric.joint_groups} if enabled_groups is None
               else dict(enabled_groups))
    if set(enabled) != set(fabric.joint_groups) or any(type(v) is not bool for v in enabled.values()):
        raise ContractError("enabled_groups must contain one boolean for every canonical group")
    out = list(fabric.parked_q or fabric.home_q)
    for name, indices in fabric.group_indices.items():
        if not enabled[name]:
            continue
        if name not in groups:
            raise ContractError(f"active group {name!r} has no input values")
        values = list(groups[name])
        if len(values) != len(indices):
            raise ContractError(f"group {name!r} has {len(values)} values, expected {len(indices)}")
...
def side_bodies(manifest: dict, side: str, ee_kind: str) -> tuple[str, list]:
    ...
    palm = (f"{p}_hl_palm_sensor" if ee_kind == "rh56f1" else
            f"{p}_hl_palm" if ee_kind == "dg5f" else f"{p}_hl_gripper_base")
    if ee_kind == "none":
        palm = f"{p}_al_7"
...
def _joint_limits(urdf: Path, joint_order: list[str]) -> tuple[list[list[float]], list[float]]:
    joints = {joint.get("name"): joint for joint in ET.parse(urdf).getroot().findall("joint")}
    position, velocity = [], []
    for name in joint_order:
        limit = joints[name].find("limit") if name in joints else None
        if (limit is None or limit.get("lower") is None or limit.get("upper") is None
                or limit.get("velocity") is None):
            raise ContractError(f"{urdf}: joint {name!r} lacks position/velocity limits")
        position.append([float(limit.get("lower")), float(limit.get("upper"))])
...
def _side_fabric(spec: AssetSpec, side: str, arm: list, hand: list, home_arm: list, home_hand: dict,
                 home_source: str, canonical_order: list | None = None,
                 canonical_home: list | None = None) -> FabricCfg | None:
    fs = spec.fabric.get(side)
    if fs is None:
        return None
    if fs.scope == "asset":
        joint_order = list(canonical_order or ())
        home_q = list(canonical_home or ())
        groups = canonical_joint_groups(joint_order)
        indices = canonical_group_indices(joint_order, groups)
        position_limits, velocity_limits = _joint_limits(spec.urdf, joint_order)
        return FabricCfg(
            class_name=fs.class_name, robot_dir=fs.robot_dir, params=fs.params, world=dict(fs.world),
            dt=CONTROL_FABRIC_DT, decimation=CONTROL_FABRIC_DECIMATION, damping=CONTROL_FABRIC_DAMPING,
            max_objects=CONTROL_MAX_OBJECTS, joint_order=joint_order, home_q=home_q, vel_ff_scale=1.0,
            home_source=home_source, joint_groups=groups, group_indices=indices, parked_q=list(home_q),
            position_limits=position_limits, velocity_limits=velocity_limits)
...
def _control_side(spec: AssetSpec, manifest: dict, side: str, home_arm: list, home_source: str,
                  gains_yaml: Path, canonical_order: list | None = None,
                  canonical_home: list | None = None) -> SideCfg:
    ...
    return SideCfg(side=side, arm_joints=arm, hand_joints=hand, ee_kind=spec.ee_kind, palm_body=palm,
                   ...
                   fabric=_side_fabric(spec, side, arm, hand, home_arm, home_h, home_source,
                                       canonical_order, canonical_home),
...
def build_asset_contract(asset: str = DEFAULT_ASSET, sides: tuple = ("right", "left"), primary: str = "right",
                         ...
                         enabled_groups: dict | None = None) -> DeployContract:
    ...
    asset_wide = any(fs is not None and fs.scope == "asset" for fs in spec.fabric.values())
    canonical_order = list(manifest["control_joint_order"]) if asset_wide else None
    canonical_home = None
    if asset_wide:
        if set(sides) != {"left", "right"}:
            raise ContractError(f"asset {asset}: its Fabric cspace covers both sides; sides must be left,right")
        per_side_joints = {side: side_joints(manifest, side) for side in sides}
        hand_homes = {side: hand_home(spec.ee_kind, side, per_side_joints[side][1]) for side in sides}
        canonical_home = []
        for joint in canonical_order:
            side = "right" if joint.startswith("r_") else "left"
            if "_aj_" in joint:
                canonical_home.append(homes[side][per_side_joints[side][0].index(joint)])
            else:
                canonical_home.append(hand_homes[side][joint])
        unknown = set(enabled_groups or ()) - set(canonical_joint_groups(canonical_order))
        if unknown:
            raise ContractError(f"asset {asset}: unknown enabled_groups {sorted(unknown)}")
    side_cfgs = {
        side: _control_side(spec, manifest, side, homes[side], home_source, gains_yaml,
                            canonical_order, canonical_home)
        for side in sides
    }
    ...
    c = DeployContract(
        ...
        enabled_groups=({name: bool((enabled_groups or {}).get(name, True))
                         for name in main.fabric.joint_groups} if main.fabric.joint_groups else {}))
