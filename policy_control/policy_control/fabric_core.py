...
class FabricCore:
    ...
    def __init__(self, contract: DeployContract, device: str, home_q=None, *, side: str | None = None,
        ...
        if f.group_indices:
            arm_group = f"{self.side}_arm"
            hand_group = f"{self.side}_hand"
            self._arm_indices = np.asarray(f.group_indices[arm_group], dtype=int)
            self._hand_indices = np.asarray(f.group_indices.get(hand_group, ()), dtype=int)
            if [f.joint_order[i] for i in self._arm_indices] != list(self.arm_joints):
                raise ContractError(f"fabric {arm_group} indices do not select the side's arm joints")
        else:
            self._arm_indices = np.arange(self.n_arm, dtype=int)
            self._hand_indices = np.arange(self.n_arm, self.n, dtype=int)
        ...
        self.n_hand = len(self._hand_indices)
        self.hand_joints = hand_command_joints(self.side_cfg) if self.n_hand else ()
        fabric_hand_joints = [f.joint_order[i] for i in self._hand_indices]
        self._hand_perm = (name_permutation(self.hand_joints, fabric_hand_joints)
                           if self.n_hand else None)
        self._hand_enabled = bool(self.contract.enabled_groups.get(f"{self.side}_hand", True))
        home = np.asarray(f.home_q if home_q is None else home_q, dtype=float).reshape(-1)
    ...
    def reset(self, q_home) -> None:
        ...
        self._park_disabled_groups()
        self.backend.fabric.default_config.copy_(self._q)

    def _park_disabled_groups(self) -> None:
        """Pin runtime-disabled canonical groups without changing the 26-D state layout."""
        if not self.contract.enabled_groups:
            return
        parked = self.cfg.parked_q or self.cfg.home_q
        parked_t = torch.tensor(parked, device=self.device, dtype=torch.float32)
        for name, enabled in self.contract.enabled_groups.items():
            if enabled:
                continue
            indices = self.cfg.group_indices[name]
            self._q[0, indices] = parked_t[indices]
            self._qd[0, indices] = 0.0
            self._qdd[0, indices] = 0.0
    ...
    def sync_hand(self, hand_q) -> None:
        ...
        if self._hand_enabled:
            q[0, self._hand_indices] = h
        else:
            parked = torch.tensor(self.cfg.parked_q or self.cfg.home_q,
                                  device=self.device, dtype=torch.float32)
            q[0, self._hand_indices] = parked[self._hand_indices]
        self._q = q
    ...
    def step(self, palm6, hand_target=None) -> JointTarget:
        ...
        return JointTarget(
            q_arm=substeps[-1, self._arm_indices].copy(),
            qd_arm=qd[self._arm_indices] * float(self.cfg.vel_ff_scale),
