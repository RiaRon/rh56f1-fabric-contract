...
def _parse(argv=None) -> argparse.Namespace:
    ...
    ap.add_argument("--inactive-groups", default="",
                    help="asset-only: comma-separated canonical groups to park (for example right_hand)")
...
def main(argv=None) -> int:
    ...
    if args.run is not None:
        ...
        inactive = {name for name in args.inactive_groups.split(",") if name}
        enabled = None if not inactive else {name: False for name in inactive}
        c = build_asset_contract(args.asset or DEFAULT_ASSET, sides=tuple(args.sides.split(",")),
                                 primary=args.primary, home=args.home, enabled_groups=enabled)
        out = args.out
