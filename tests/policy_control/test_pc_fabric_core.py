...
RH56F1_JSON = SIM2REAL / "logs/policy/asset_openarm_rh56f1_bi_rl/deploy_contract.json"
needs_asset = pytest.mark.skipif(not ASSET_JSON.exists(), reason="asset_openarm_dg5f-m_bi_rl contract 없음")
needs_rh56f1 = pytest.mark.skipif(not RH56F1_JSON.exists(), reason="asset_openarm_rh56f1_bi_rl contract 없음")
...
@needs_rh56f1
@pytest.mark.parametrize("side,arm_slice,hand_slice", [
    ("right", slice(0, 7), slice(7, 13)),
    ("left", slice(13, 20), slice(20, 26)),
])
def test_rh56f1_uses_name_derived_26d_group_indices(side, arm_slice, hand_slice):
    contract = C.load_contract(RH56F1_JSON)
    active = dataclasses.replace(
        contract, enabled_groups={**contract.enabled_groups, f"{side}_hand": True})
    backend = _backend(26)
    core = F.FabricCore(active, "cpu", side=side, backend=backend)
    hand = np.linspace(0.1, 0.6, 6)
    out = core.step(np.zeros(6), hand_target=hand)

    np.testing.assert_allclose(backend.fabric.calls[0]["q"].numpy()[0, hand_slice], hand, atol=1e-6)
    np.testing.assert_allclose(out.q_arm, out.q_full[arm_slice])
    assert core.n == 26 and core.n_arm == 7 and core.n_hand == 6
    assert out.q_full.shape == (26,) and out.substeps.shape == (contract.fabric.decimation, 26)


@needs_rh56f1
def test_rh56f1_inactive_right_hand_stays_parked_after_integration():
    contract = C.load_contract(RH56F1_JSON)
    backend = _backend(26)
    core = F.FabricCore(contract, "cpu", side="right", backend=backend)
    hand = np.linspace(1.0, 6.0, 6)
    out = core.step(np.zeros(6), hand_target=hand)
    indices = contract.fabric.group_indices["right_hand"]

    np.testing.assert_allclose(backend.fabric.calls[0]["q"].numpy()[0, indices],
                               np.asarray(contract.fabric.parked_q)[indices], atol=1e-6)
    np.testing.assert_allclose(out.q_full[indices], np.asarray(contract.fabric.parked_q)[indices], atol=1e-6)
    np.testing.assert_allclose(core.qd[indices], 0.0, atol=1e-6)
